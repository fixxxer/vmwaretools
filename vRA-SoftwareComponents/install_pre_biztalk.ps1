...
Write-Host "ScheduledTaskAdapter.msi ..."
#$command14 = "\\$MountPath\BiztalkPrevFiles\ScheduledTaskAdapter.msi /q"
#Invoke-Expression -Command $command14 | Out-Null

$process = "\\$MountPath\BiztalkPrevFiles\ScheduledTaskAdapter.msi"
$arguments = "\quiet"
Try 
     {
     Start-Process $process -ArgumentList $arguments -PassThru -Wait -ErrorAction Stop
...
Catch
     {
     Write-Error "Failed to run ScheduledTaskAdapter.msi"
     Write-Error $_.Exception
     Exit -1 
  }
